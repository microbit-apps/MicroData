namespace microdata {
  ...
  export class SensorScheduler {
    ...
    public stop() {
      ...
      // Force unregistry of all buttons:
    ...
    */
    public start(callbackObj?: ITargetDataLoggedCallback) {
      ...
      control.inBackground(() => {
        let lastLogTime = control.millis();
        let currentTime = 0;
        ...
        while (this.schedule.length > 0) {
          ...
          nowMs = control.millis();
          // const pauseTime = Math.max(1, sleepTime - (nowMs - lastLogTime));  // Discount for operation time

          const loopCostMS = 8;
          const pauseTime = Math.max(sleepTime - loopCostMS, 0);
          control.dmesg(`pt: ${(pauseTime)} ${(nowMs - lastLogTime) - sleepTime}`)
          // basic.pause(pauseTime);

          lastLogTime = nowMs;
          currentTime += sleepTime

          for (let i = 0; i < pauseTime; i += 10) {
          ...
          control.dmesg(`logTime: ${(control.micros() - beforeUs)}`)
        }
