namespace microdata {
    ...
    export class IntervalMode extends Scene {
        ...
        /* override */ startup() {
                ...
                () => {
                    ...
                    if (this.guiStateQuestion == GUI_STATE_QUESTION.ASKING_HOW_LONG) {
                        ...
                        if (this.guiStateAnswer == GUI_STATE_USER_ANSWER.CHANGING_EXPERIMENT_INTERVAL) {
                            // Limit the interval if it has the same units as the question does (if total length is 1 minute, the interval must be 1 minute or less)
                            if (this.experimentLengthUnitsIndex == this.experimentIntervalUnitsIndex)
                                this.experimentInterval = Math.min(this.experimentInterval + 1, this.experimentLength)
                            else
                                this.experimentInterval += 1;
                        } else {
                            const len = this.experimentLengthUnitsIndex + 1
                            this.experimentIntervalUnitsIndex = (this.experimentIntervalUnitsIndex + 1) % len;
                        }
                    }
                        // basic.pause(200)
                    // }
            ...
            context.onEvent(
                ...
                () => {
                    ...
                    if (this.guiStateQuestion == GUI_STATE_QUESTION.ASKING_HOW_LONG) {
                        ...
                        // Limit the amount you can change so that the interval is always <= experiment length.
                        if (this.guiStateAnswer == GUI_STATE_USER_ANSWER.CHANGING_EXPERIMENT_INTERVAL) {
            ...
            context.onEvent(
                ...
                () => {
                    if (this.guiStateQuestion == GUI_STATE_QUESTION.ASKING_HOW_LONG) {
                        ...
                        // basic.showNumber(this.experimentInterval)
                        // basic.showNumber(EXPERIMENT_UNITS_CONVERSION_TABLE[this.experimentIntervalUnitsIndex])
                        const period_ms: number = this.experimentInterval * EXPERIMENT_UNITS_CONVERSION_TABLE[this.experimentIntervalUnitsIndex];
                        const sensorCfg: RecordingConfig = {measurements: this.experimentLength, period: period_ms}

                        if (this.nextSceneEnum == MicroDataSceneEnum.DistributedLogging) {
                            this.app.pushScene(new DistributedLoggingScreen(this.app, this.sensors, this.sensors.map(_ => sensorCfg))); // Temp disabled with Distributedlogging (no mem)
                        } else {
                            // All sensors are configured, pass them their config and move to the DataRecording screen:
                            this.sensors.map(sensor => {
                                sensor.setConfig(sensorCfg)
                            });
            ...
            context.onEvent(
                ...
                () => {
                    if (this.guiStateQuestion == GUI_STATE_QUESTION.ASKING_HOW_LONG) {
                        this.app.popScene();
                        this.app.pushScene(new LoggingModeSelection(this.app, this.sensors, this.nextSceneEnum))
                    } else {
        ...
        draw() {
            ...
            const sensorName = "Experiment setup";
            ...
            // Draw cursor to show what is selected:

            const correctAnswer = (this.guiStateQuestion == GUI_STATE_QUESTION.ASKING_HOW_LONG) ? GUI_STATE_USER_ANSWER.CHANGING_EXPERIMENT_LENGTH : GUI_STATE_USER_ANSWER.CHANGING_EXPERIMENT_INTERVAL
            const cursorX = (this.guiStateAnswer == correctAnswer) ? xCenteringOffset - 4 : xCenteringOffset + firstColBoundaryWidth + colSpacingWidth
            const cursorWidth = (this.guiStateAnswer == correctAnswer) ? firstColBoundaryWidth : secondColBoundaryWidth + 2

            const cursorY = (screen().height >> 1) - (textFont.charHeight >> 1) + 1
            for (let i = 0; i < 2; i++) {
                screen().drawRect(
                    cursorX - i,
                    cursorY - i - 2,
                    cursorWidth + (2 * i),
                    textFont.charHeight + 4 + (2 * i),
                    6
                ) // End of Second col
            }
    ...
    export class LoggingModeSelection extends CursorScene {
        ...
        private readonly nextSceneEnum: MicroDataSceneEnum;
        constructor(app: AppInterface, sensors: Sensor[], nextSceneEnum?: MicroDataSceneEnum) {
            super(app);
            this.sensors = sensors;
            this.nextSceneEnum = nextSceneEnum;
        ...
        /* override */ startup() {
            ...
            this.navigator.setBtns([[
                new Button({
                    ...
                    onClick: () => {
                        this.app.popScene()
                        this.app.pushScene(new IntervalMode(this.app, this.sensors, this.nextSceneEnum))
                    },
                }),
